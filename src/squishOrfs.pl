...
#    squishOrfs.pl
#    
#    Squish together orfs from the same contig into a single sequence
#
...
# CODE HERE
######################################################################
my $outfile = openWrite($global_options->{'out'});
my $current_seq_header = "unset";
my $current_seq = "";
...
sub checkParams {
    ...
    #
    my @standard_options = ( "help|h+", "in|i:s", "out|o:s");
    my %options;
    ...
    # Compulsory items
    if(!exists $options{'in'} ) { printParamError ("You need to supply an input file"); }
    if(!exists $options{'out'} ) { printParamError ("You need to supply an output file"); }
...
=head1 DESCRIPTION
   ...
   Squish together orfs from the same contig into a single sequence
   You'll need to make sure that all orfs from the same contig have the 
   same header.

=head1 SYNOPSIS

    squishOrfs.pl -in|i INFILE -out|o OUTFILE

      -in -i INFILE                File of genes 
      -out -o OUTFILE              File to write to
